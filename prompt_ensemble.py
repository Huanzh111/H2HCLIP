...
class AnomalyCLIP_PromptLearner(nn.Module):
    def __init__(self, clip_model, design_details):
        ...
        #
        key_words_pos_region = "regions perfect smooth intact uniform clean unmarked regular consistent"
        key_words_neg_region = "regions flawed crack scratch rough pitted warped broken scratched"
        ...
        prompt_pos_region = [prompt_prefix_pos + " " + key_words_pos_region + "."]
        prompt_neg_region = [prompt_prefix_neg + " " + key_words_neg_region + "."]
        ...
        tokenized_prompts_pos_region = []
        tokenized_prompts_neg_region = []
        for p_pos_region in prompt_pos_region:
            tokenized_prompts_pos_region.append(tokenize(p_pos_region))
        for p_neg_region in prompt_neg_region:
            tokenized_prompts_neg_region.append(tokenize(p_neg_region))
        ...
        tokenized_prompts_pos_region = torch.cat(tokenized_prompts_pos_region)
        tokenized_prompts_neg_region = torch.cat(tokenized_prompts_neg_region)
        ...
        # 生成相应的text embedding
        with torch.no_grad():
            embedding_pos_region = clip_model.token_embedding(tokenized_prompts_pos_region).type(dtype)
            embedding_neg_region = clip_model.token_embedding(tokenized_prompts_neg_region).type(dtype)
            ...
            n3, l3, d3 = embedding_pos_region.shape
            embedding_pos_region = embedding_pos_region.reshape(normal_num, self.n_cls, l3, d3).permute(1, 0, 2, 3)
            embedding_neg_region = embedding_neg_region.reshape(anormaly_num, self.n_cls, l3, d3).permute(1, 0, 2, 3)
        ...
        self.register_buffer("token_prefix_pos_region", embedding_pos_region[:, :, :1, :])  # torch.Size([1, 1, 1, 768])
        self.register_buffer("token_suffix_pos_region", embedding_pos_region[:, :, 1 + n_ctx_pos:, :])  # torch.Size([1, 1, 64, 768])
        self.register_buffer("token_prefix_neg_region", embedding_neg_region[:, :, :1, :])  # torch.Size([1, 1, 1, 768])
        self.register_buffer("token_suffix_neg_region", embedding_neg_region[:, :, 1 + n_ctx_neg:, :])  # torch.Size([1, 1, 64, 768])
        ...
        n3, d3 = tokenized_prompts_pos_region.shape
        tokenized_prompts_pos_region = tokenized_prompts_pos_region.reshape(normal_num, self.n_cls, d3).permute(1, 0, 2)
        ...
        n3, d3 = tokenized_prompts_neg_region.shape
        tokenized_prompts_neg_region = tokenized_prompts_neg_region.reshape(anormaly_num, self.n_cls, d3).permute(1, 0, 2)
        ...
        self.register_buffer("tokenized_prompts_pos_region", tokenized_prompts_pos_region)
        self.register_buffer("tokenized_prompts_neg_region", tokenized_prompts_neg_region)
    ...
    def forward(self, cls_id=None):
        ...
        prefix_pos_region = self.token_prefix_pos_region
        prefix_neg_region = self.token_prefix_neg_region
        suffix_pos_region = self.token_suffix_pos_region
        suffix_neg_region = self.token_suffix_neg_region
        ...
        prompts_pos_region = torch.cat(
            [
                # N(the number of template), 1, dim
                prefix_pos_region,  # (n_cls, 1, dim)
                ctx_pos,  # (n_cls, n_ctx, dim)
                suffix_pos_region,  # (n_cls, *, dim)
            ], dim=2, )
        prompts_neg_region = torch.cat(
            [
                prefix_neg_region,  # (n_cls, 1, dim)
                ctx_neg,  # (n_cls, n_ctx, dim)
                suffix_neg_region,  # (n_cls, *, dim)
            ], dim=2, )
        ...
        _, _, l3, d3 = prompts_pos_region.shape
        prompts_pos_region = prompts_pos_region.reshape(-1, l3, d3)
        _, _, l3, d3 = prompts_neg_region.shape
        prompts_neg_region = prompts_neg_region.reshape(-1, l3, d3)
        prompts_region = torch.cat([prompts_pos_region, prompts_neg_region], dim=0)
        ...
        _, l3, d3 = self.tokenized_prompts_pos_region.shape
        tokenized_prompts_pos_region = self.tokenized_prompts_pos_region.reshape(-1, d3)
        _, l3, d3 = self.tokenized_prompts_neg_region.shape
        tokenized_prompts_neg_region = self.tokenized_prompts_neg_region.reshape(-1, d3)
        tokenized_prompts_region = torch.cat((tokenized_prompts_pos_region, tokenized_prompts_neg_region), dim=0)
        ...
        return prompts, tokenized_prompts, prompts_emo, tokenized_prompts_emo, prompts_class, tokenized_prompts_class, prompts_region, tokenized_prompts_region, self.compound_prompts_text