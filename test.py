...
def multi_scale(patch_feature):
    ...
    high_res_features = high_res_features[:, :, :h].transpose(1, 2)  # 裁剪到原始长度
    # 低分辨率特征：
    ...
    # fre_feature = torch.fft.fft(patch_feature).abs()
    # fre_feature_avg = F.adaptive_max_pool1d(fre_feature.transpose(1, 2), output_size=384).transpose(1, 2)
    ...
    # ***********************n************
    attention = fus_attention + attention + res_features_fus
    attention = torch.sigmoid(attention)
    # 使用线性插值将低分辨率特征上采样回到原始大小（1226, 768）
    ...
    alpha = 0.01  # 高分辨率特征权重
    beta = 0.99  # 低分辨率特征权重
    fused_features_sum = alpha * high_res_features + beta * low_res_features_upsampled
...
def test(args):
    ...
    for idx, items in enumerate(tqdm(test_dataloader)):
        ...
        with torch.no_grad():
            ...
            text_probs_emo = image_features @ text_features_emo_yuan.permute(0, 2, 1)
            text_probs_emo = (text_probs_emo / 0.07).softmax(-1)
            text_probs_emo = text_probs_emo[:, 0, 1]

            text_probs_fusion = image_features.unsqueeze(1) @ text_features_fustion.permute(0, 2, 1)
            text_probs_fusion = (text_probs_fusion / 0.07).softmax(-1)
            text_probs_fusion = text_probs_fusion[:, 0, 1]
            text_probs = (text_probs_emo + text_probs_yuan + text_probs_fusion + text_probs_class) / 4.0

            anomaly_map_list = []
            for idx, patch_feature in enumerate(patch_features):
                if idx >= args.feature_map_layer[0]:
                    patch_feature = patch_feature / patch_feature.norm(dim=-1, keepdim=True)
                    patch_feature_fus = multi_scale(patch_feature) * 0.2 + patch_feature * 0.8
                    ...
                    frequency_features = frequency_features.cuda()
                    similarity_fre, _ = AnomalyCLIP_lib.compute_similarity(frequency_features, text_features_fustion[0])
                    ...
                    similarity, _ = AnomalyCLIP_lib.compute_similarity(patch_feature_fus, text_features_fustion[0])
                ...
                # if idx >= args.feature_map_layer[0]:
                #     patch_feature = patch_feature / patch_feature.norm(dim=-1, keepdim=True)
                #     similarity, _ = AnomalyCLIP_lib.compute_similarity(patch_feature, text_features_fustion[0])
                #     similarity_map = AnomalyCLIP_lib.get_similarity_map(similarity[:, 1:, :], args.image_size)
                #     anomaly_map = (similarity_map[..., 1] + 1 - similarity_map[..., 0]) / 2.0
                #     anomaly_map_list.append(anomaly_map)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--checkpoint_path", type=str, default='./mvtec_model/epoch_7.pth', help='path to checkpoint')
    parser.add_argument("--dataset", type=str, default='mvtec')
    parser.add_argument("--features_list", type=int, nargs="+", default=[6, 12, 18, 24], help="features used")
    parser.add_argument("--image_size", type=int, default=410, help="image size")  # image 400 pixel 410
