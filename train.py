...
def fused_feature(text_features_emo, text_features, text_features_class, text_features_region):
    ...
    text_features_region_normal = text_features_region[0].cpu().detach().numpy()
    # local feature
    coll_region = np.corrcoef(text_features_region_normal, text_features_emo_normal)
    threshold_region = coll_region[0][1]
    feaure_normal_region = np.vstack([text_features_emo_normal, text_features_region_normal])
    fused_feature_normal_region = np.mean(feaure_normal_region, axis=0) * threshold_region
    # global feature
    ...
    fused_feature_normal = np.mean(feaure_normal, axis=0) * threshold
    final_fusion_normal = fused_feature_normal + text_features_class_normal + fused_feature_normal_region
    ...
    text_features_region_abnormal = text_features_region[1].cpu().detach().numpy()

    # local
    coll_region = np.corrcoef(text_features_region_abnormal, text_features_emo_abnormal)
    threshold_region = coll_region[0][1]
    feaure_abnormal_region = np.vstack([text_features_emo_abnormal, text_features_region_abnormal])
    fused_feature_abnormal_region = np.mean(feaure_abnormal_region, axis=0) * threshold_region
    # global
    ...
    fused_feature_abnormal = np.mean(feaure_abnormal, axis=0) * threshold
    final_fusion_abnormal = fused_feature_abnormal + text_features_class_abnormal + fused_feature_abnormal_region
...
def random_compare(text_probs, text_probs_fusion, label):
    ...
    if torch.rand(1).item() < eps:
        ...
        select_prob_1_image = text_probs_fusion[random_index_1]
        select_prob_2 = text_probs[random_index_2]
        select_label_1 = label[random_index_1]
        select_label_2 = label[random_index_2]
        select_prob_2_image = text_probs_fusion[random_index_2]
    else:
        select_prob_1 = text_probs[0]
        select_prob_1_image = text_probs_fusion[0]
    ...
    if select_label_1.item() == 1 and select_label_2.item() == 1:
        dis = torch.abs(select_prob_1_image[1] - select_prob_2_image[1])
        loss = 0.5 * (dis ** 2)
    if select_label_1.item() == 0 and select_label_2.item() == 0:
        dis = torch.abs(select_prob_1_image[0] - select_prob_2_image[0])
        loss = 0.5 * (dis ** 2)
    if select_label_1.item() == 1 and select_label_2.item() == 0:
        image_dis = torch.abs(select_prob_1_image[0] - select_prob_2_image[0])
        pixel_dis = torch.abs(select_prob_1[0] - select_prob_2[0])
        loss = F.relu((1.0 - image_dis) ** 2) * 0.5 + F.relu((1.0 - pixel_dis) ** 2) * 0.1
    if select_label_1.item() == 0 and select_label_2.item() == 1:
        image_dis = torch.abs(select_prob_1_image[1] - select_prob_2_image[1])
        pixel_dis = torch.abs(select_prob_1[0] - select_prob_2[0])
        loss = F.relu((1.0 - image_dis) ** 2) * 0.5 + F.relu((1.0 - pixel_dis) ** 2) * 0.1
    return loss
...
def train(args):
    ...
    for epoch in tqdm(range(args.epoch)):
        ...
        for items in tqdm(train_dataloader):
            ...
            #########################################################################
            prompts, tokenized_prompts, prompts_emo, tokenized_prompts_emo, prompts_class, tokenized_prompts_class, prompts_region, tokenized_prompts_region, compound_prompts_text = prompt_learner(
                cls_id=None)
            text_features_region = model.encode_text_learn(prompts_region, tokenized_prompts_region,compound_prompts_text).float()
            text_features_region_yuan = torch.stack(torch.chunk(text_features_region, dim=0, chunks=2), dim=1)
            text_features_region_yuan = text_features_region_yuan / text_features_region_yuan.norm(dim=-1, keepdim=True)
            ...
            text_features_fustion = fused_feature(text_features_emo, text_features, text_features_class, text_features_region)
            ...
            for idx, patch_feature in enumerate(patch_features):
                if idx >= args.feature_map_layer[0]:
                    ...
                    similarity, _ = AnomalyCLIP_lib.compute_similarity(patch_feature, text_features_region_yuan[0])
                    # torch.Size([8, 1370, 2])
                    similarity_map = AnomalyCLIP_lib.get_similarity_map(similarity[:, 1:, :], args.image_size).permute(0, 3, 1, 2)

                    similarity_map_list.append(similarity_map)
                    similarity_emo, _ = AnomalyCLIP_lib.compute_similarity(patch_feature, text_features_emo_yuan[0])
                    similarity_map_emo = AnomalyCLIP_lib.get_similarity_map(similarity_emo[:, 1:, :], args.image_size).permute(0, 3, 1, 2)
            ...
            interaction_loss = 0
            for i in range(len(emo_map_list)):
                pixel_to_image = torch.mean(similarity_map_list[i][:, 1, :, :], dim=(1, 2), keepdim=True).squeeze(2)
                interaction_loss = interaction_loss + random_compare(pixel_to_image, text_probs_emo, label)

            image_loss_total = image_loss + image_loss_emo + image_loss_fusion + image_loss_class
            pixel_loss = loss + loss_emo
            total_loss = pixel_loss + image_loss_total + 0.001 * interaction_loss
            optimizer.zero_grad()
            total_loss.backward()
            optimizer.step()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--train_data_path", type=str, default="./mvtec/mvtec", help="train dataset path")
    parser.add_argument("--save_path", type=str, default='./test_nocare', help='path to save results')
